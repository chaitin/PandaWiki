...
2. 进入本目录并安装依赖：
   ...
   npm install
   ```
   > 依赖 `@panda-wiki/icons`、`@panda-wiki/themes`、`@panda-wiki/ui` 通过 npm workspaces（`../packages/*`）链接，必须在 `web/app` 目录下执行安装。
3. 配置环境变量：
   - 在本目录下新建 `.env.local`，根据需求修改环境变量，例如：
...
- `npm run build`: 构建生产版本（使用 webpack，兼容 Windows standalone 输出）
- `npm start`: 启动生产服务器
- `npm run lint`: 代码检查
- `npm run api`: 生成 API 文档（环境变量需提供 `SWAGGER_BASE_URL`、`SWAGGER_AUTH_TOKEN`）
